 ...
 */class CrawlerRepository{/**
     ...
     */findAllUrls(progress){var _this=this;return _asyncToGenerator(function*(){_this.progress=progress;if(_this.crawlStatesRepository.urlsPoolSize()===0){_this.crawlStatesRepository.addPoolUrl(_this.initialUrl)}_this.browser=yield _puppeteer.default.launch({ignoreHTTPSErrors:true,headless:true});_this.page=yield _this.browser.newPage();_this.page.setJavaScriptEnabled(true);return new Promise(resolve=>{_this.resolve=resolve;_this.crawlNextUrl()})})()}/**
     * Goto the page and get the html contents of the page.
     * @param url of the page.
     * @returns {Promise<void>}
     */processPage(url){var _this2=this;return _asyncToGenerator(function*(){yield _this2.page.goto(url,{"waitUntil":"networkidle2"});const waitForRender=_this2.option.index.waitForRender;if(waitForRender){yield _this2.page.waitFor(waitForRender)}const response=yield(0,_axios.default)(url);const content=yield _this2.page.mainFrame().content();return{headers:response.headers,html:content}})()}/**
     * Gets the page, if there are more pages it will add them to the list
     * else, just adds the urls to the urls array.
     */crawlNextUrl(){var _this3=this;const urlsPoolSize=this.crawlStatesRepository.urlsPoolSize();if(urlsPoolSize===0){this.browser.close();return this.resolve(this.crawlStatesRepository.findAllUrls())}let url=CrawlerRepository.cleanUrl(this.crawlStatesRepository.popPoolUrl());this.processPage(url).then(/*#__PURE__*/function(){var _ref=_asyncToGenerator(function*(data){const newUrl=new _Url.default(url);_this3.crawlStatesRepository.addUrl(newUrl);const urlsSize=_this3.crawlStatesRepository.urlsSize();_this3.progress(new _Progress.default(newUrl,data.html,data.headers,urlsSize,urlsPoolSize-1));if(_this3.args.isSingle()){_this3.resolve(_this3.crawlStatesRepository.findAllUrls())}else{const links=yield _this3.page.$$("a");for(let linkHandle of links){const href=yield _this3.page.evaluate(link=>link.href,linkHandle);let foundUrl=CrawlerRepository.cleanUrl(href);if(_this3.isFreshUrl(foundUrl)){_this3.crawlStatesRepository.addPoolUrl(foundUrl)}}_this3.crawlNextUrl()}});return function(_x){return _ref.apply(this,arguments)}}()).catch(()=>{this.crawlNextUrl()})}/**
     ...
     */isNotExclusion(url){let _UrlParser$parse=_url.default.parse(url),path=_UrlParser$parse.path;for(let exclusion of this.option.index.exclusions){if(path.startsWith(exclusion)){return false}}return true}/**
