...
class HeadersRepository {
  ...
  save(url, headers) {
    const file = (0, _path.join)(this.getProjectsHeadersFolder(), url.name + ".json");
    return new Promise(resolve => {
      (0, _fs.writeFileSync)(file, JSON.stringify(headers));
      resolve();
    });
  }
  /**
  ...
  getProjectsHeadersFolder() {
    let projectsPathHeaders = (0, _path.join)(this.projectFolder, "headers");

    if (!(0, _fs.existsSync)(projectsPathHeaders)) {
      (0, _fs.mkdirSync)(projectsPathHeaders);
    }
...
exports.default = HeadersRepository;
