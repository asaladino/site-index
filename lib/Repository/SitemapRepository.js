 ...
 */class SitemapRepository{/**
     ...
     */parseSiteMap(){let url=this.sitemaps.pop();this.progress(new _Progress.default(new _Url.default(url),null,this.sitemaps.length,this.urls.length));(0,_thenRequest.default)("GET",url).done(res=>{let xml=res.getBody("utf8");(0,_xml2js.parseString)(xml,(err,result)=>{if(result["urlset"]){result["urlset"]["url"].forEach(entry=>{let url=new _Url.default(entry["loc"][0]);this.urls.push(url)});if(this.sitemaps.length===0){this.resolve(this.urls)}else{this.parseSiteMap()}}if(result["sitemapindex"]){result["sitemapindex"]["sitemap"].forEach(entry=>{this.sitemaps.push(entry["loc"][0])});this.parseSiteMap()}})})}}exports.default=SitemapRepository;
